...
# -------------------------------------------------
# 1) 建立一個用來「合併 6 張圖片特徵」的 Attention 模組
...
class ConvolutionalVisionTransformer(nn.Module):
    ...
    # -------------------------------------------------
    # 3) 我們示範：一次同時輸入 6 張圖像
    #    會先把 6 張圖各自透過所有 stage，得到最終的 feature (cls 或 avg)
    #    再用 merge_attn 合併
    # -------------------------------------------------
    def forward(self, x):
        """
        ...
        # 注意：視需求可在這裡套用相同 stage、或每張圖有不同 stage
        # 以下寫法是「相同 stage」，即所有 6 張圖參數共享。
        for i in range(num_views):
            # 取第 i 張圖片 => [B, 1, 3, H, W] -> squeeze 後 => [B, 3, H, W]
            single_img = x[:, i, :, :, :]
        ...
        out = self.head(merged_feat)
        return out

    # -------------------------------------------------
    # 4) forward_features：把「單張圖片」跑過多個 stage
    #    此邏輯和原本單張圖片版類似
    # -------------------------------------------------
    def forward_features(self, x):
        ...
        if self.cls_token:
            ...
            # cls_tokens 是 [B, 1, D] => squeeze => [B, D]
...
# -------------------------------------------------
# 其餘如 VisionTransformer, Block 等維持原本即可
# 只要注意 ConvolutionalVisionTransformer 中的 forward
# 讓它可以處理 [B, 6, 3, H, W] 即可。
# -------------------------------------------------
